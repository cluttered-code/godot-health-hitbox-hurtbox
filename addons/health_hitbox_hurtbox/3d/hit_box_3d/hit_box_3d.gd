...
## [HitBox3D] is and abstract class associated with an object that can collide with a [HurtBox3D].
...
## Emitted after actions are applied to a [HurtBox3D].
...
## Detect collisions with [HitBox3D], [HurtBox3D], or [Area3D] and apply appropriate action.
